...
float earthRadius = 6.371f; // M
float moonRadius = 1.737f; // M
...
static void render(void) {
  ...
  model = glm::scale(model, glm::vec3(scaleFactor*earthRadius, scaleFactor*earthRadius, scaleFactor*earthRadius));
  ...
  model = glm::rotate(model, glm::radians(yAngle), glm::vec3(0, 1, 0)); // rotate about the y-axis
  model = glm::rotate(model, glm::radians(zAngle), glm::vec3(0, 0, 1)); // rotate about the z-axis
  model = glm::scale(model, glm::vec3(scaleFactor*moonRadius, scaleFactor*moonRadius, scaleFactor*moonRadius));
  model = glm::translate(model, glm::vec3(distancefromearth, 0.0f, 0.0f));

  // model-view-projection matrix
  mvp = projection * view * model;
  mvpMatrixId = glGetUniformLocation(programId, "u_MVP");
  glUniformMatrix4fv(mvpMatrixId, 1, GL_FALSE, &mvp[0][0]);

  // find the names (ids) of each vertex attribute
  positionAttribId = glGetAttribLocation(programId, "position");
  textureCoordsAttribId = glGetAttribLocation(programId, "textureCoords");
  normalAttribId = glGetAttribLocation(programId, "normal");
